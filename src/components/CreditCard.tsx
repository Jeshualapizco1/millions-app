  ...
  return (
    <div onClick={() => onEdit(credit)} style={{ background: C.card, border: `1px solid ${C.border}22`, borderRadius: 20, padding: 18, marginBottom: 12, cursor: "pointer", borderLeft: `4px solid ${type.color}` }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {credit.type === "tarjeta" && credit.cut_day && <div style={{ flex: 1, background: C.surface, borderRadius: 10, padding: "8px 10px", textAlign: "center" }}><div style={{ fontSize: 11, color: C.muted }}>Corte día {credit.cut_day}</div><div style={{ fontSize: 13, fontWeight: 700, color: uc(daysCorte) }}>{daysCorte === 0 ? "¡Hoy!" : daysCorte === 1 ? "Mañana" : `${daysCorte}d`}</div></div>}
        {credit.type === "tarjeta" && credit.payment_day && <div style={{ flex: 1, background: C.surface, borderRadius: 10, padding: "8px 10px", textAlign: "center" }}><div style={{ fontSize: 11, color: C.muted }}>Pago día {credit.payment_day}</div><div style={{ fontSize: 13, fontWeight: 700, color: uc(daysPago) }}>{daysPago === 0 ? "¡Hoy!" : daysPago === 1 ? "Mañana" : `${daysPago}d`}</div></div>}
        {credit.type !== "tarjeta" && credit.next_payment_date && <div style={{ flex: 1, background: C.surface, borderRadius: 10, padding: "8px 10px", textAlign: "center" }}><div style={{ fontSize: 11, color: C.muted }}>Próximo pago</div><div style={{ fontSize: 12, fontWeight: 700, color: uc(daysNext) }}>{daysNext! <= 0 ? "¡Vencido!" : daysNext === 1 ? "Mañana" : `${daysNext}d`}</div></div>}
      </div>
      {onPay && debt > 0 && (
        <button
