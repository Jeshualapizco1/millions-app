  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 0 }}>
        <div style={{ flex: 1 }}><label style={S.lbl}>Meta ($)</label><input style={{ ...S.inp, marginBottom: 12 }} type="number" inputMode="decimal" placeholder={isNew ? "0.00" : undefined} value={form.target_amount} onChange={(e) => update({ target_amount: e.target.value })} /></div>
        <div style={{ flex: 1 }}><label style={S.lbl}>{isNew ? "Ya tengo ($)" : "Ahorrado ($)"}</label><input style={{ ...S.inp, marginBottom: 12 }} type="number" inputMode="decimal" placeholder={isNew ? "0.00" : undefined} value={form.current_amount} onChange={(e) => update({ current_amount: e.target.value })} /></div>
      </div>
      <label style={S.lbl}>{isNew ? "Fecha objetivo (opcional)" : "Fecha objetivo"}</label>
      <input style={{ ...S.inp, marginBottom: 12 }} type="date" value={form.target_date || ""} onChange={(e) => update({ target_date: e.target.value })} />
