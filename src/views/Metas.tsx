  ...
  return (
    <div className="fadeUp">
      ...
      <div style={S.card}>
        ...
        {recurring.length === 0 && (
          <div style={{ color: C.muted, fontSize: T.md, textAlign: "center", padding: 16 }}>
            ...
            <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 0", borderBottom: `1px solid ${C.border}22`, opacity: r.active ? 1 : 0.5 }}>
              <div onClick={() => onEditRecurring(r)} style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0, cursor: "pointer" }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: T.base, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.name}</div>
      ...
      <div id="presupuestos" style={{ ...S.card, scrollMarginTop: 90 }}>
        ...
        <div onClick={onSetTotalBudget} style={{ background: C.surface, borderRadius: 14, padding: "12px 14px", marginBottom: 16, cursor: "pointer", border: `1px solid ${totalBudget?.willExceed ? C.red + "55" : C.border + "44"}` }}>
          {totalBudget ? (
            ...
            <div style={{ fontSize: T.md, color: C.muted }}>＋ Definir un techo de gasto para todo el mes</div>
          )}
        </div>

        {budgetProgress.length === 0 && <div style={{ color: C.muted, fontSize: T.md, textAlign: "center", padding: 16 }}>Sin presupuestos por categoría. Define cuánto puedes gastar en cada una.</div>}
            ...
            <div key={b.id} style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: T.lg }}>{look(b.category).icon}</span>
      ...
      <div style={S.card}>
        ...
        {goals.length === 0 && <div style={{ color: C.muted, fontSize: T.md, textAlign: "center", padding: 16 }}>Sin metas aún. Define tus objetivos financieros.</div>}
        {goals.map((g) => {
            ...
            <div key={g.id} style={{ background: C.surface, borderRadius: 16, padding: 16, marginBottom: 12, border: `1px solid ${g.color}33` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 40, height: 40, borderRadius: R.md, background: g.color + "22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>{g.icon}</div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 15 }}>{g.name}</div>
                    {targetDate && <div style={{ fontSize: T.xs, color: daysLeft! <= 30 ? C.amber : C.muted }}>{daysLeft! <= 0 ? "¡Fecha vencida!" : daysLeft === 1 ? "Mañana" : `${daysLeft} días restantes`}</div>}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 6 }}>
