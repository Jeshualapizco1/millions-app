...
export default function Asistente({
  ...
  useEffect(() => { endRef.current?.scrollIntoView({ behavior: reducedMotion() ? "auto" : "smooth" }); }, [aiMsgs]);
  return (
    <div className="fadeUp">
      <div>

        <div style={{ fontSize: T.md, color: C.muted, marginBottom: uso ? 6 : 14 }}>Pregunta sobre tus cuentas, movimientos y planes. Tú confirmas cualquier cambio.</div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
          {QUICK_QUESTIONS.map((q) => (
            <button key={q} onClick={() => onSend(q)} disabled={aiLoading || !!uso?.agotado} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: R.lg, padding: "6px 12px", fontSize: T.sm, color: C.aLight, cursor: "pointer" }}>{q}</button>
          ))}
        </div>
        <div role="log" aria-label="Conversación con el asistente" aria-live="polite" style={{ display: "flex", flexDirection: "column", gap: 10, minHeight: 200, maxHeight: "min(52dvh,540px)", overflowY: "auto", marginBottom: 14 }}>
          {aiMsgs.map((m, i) => (
            <div key={i} className="msg" style={{ display: "flex", flexDirection: "column", alignItems: m.role === "user" ? "flex-end" : "flex-start", gap: 8 }}>
              <div style={{ maxWidth: "85%", background: m.role === "user" ? C.accent : C.surface, border: m.role === "assistant" ? `1px solid ${C.border}44` : "none", borderRadius: m.role === "user" ? "18px 18px 4px 18px" : "18px 18px 18px 4px", padding: "10px 14px", color: m.role === "user" ? "#fff" : C.text, fontSize: T.base, lineHeight: 1.6, overflowWrap: "anywhere", whiteSpace: "pre-wrap" }}>{m.text}</div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input aria-label="Pregunta al asistente" style={{ ...S.inp, flex: 1 }} placeholder="Pregunta sobre tus finanzas…" value={aiInput} onChange={(e) => setAiInput(e.target.value)} onKeyDown={(e) => { if(e.key === "Enter" && !e.nativeEvent.isComposing && !aiLoading && !uso?.agotado) onSend(aiInput.trim()); }} />
          <button style={{ ...S.btn(), padding: "12px 16px" }} onClick={() => onSend(aiInput.trim())} aria-label="Enviar pregunta" disabled={aiLoading || !aiInput.trim() || !!uso?.agotado}><Icon name="flecha"/></button>
        </div>
      </div>
    </div>
  );
